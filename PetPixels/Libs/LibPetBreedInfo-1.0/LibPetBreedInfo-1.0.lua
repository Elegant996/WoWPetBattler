...
function lib:GetBreedByStats(speciesID,level,rarity,health,power,speed)
	...
	if not self.breedData.speciesToBaseStats[speciesID] or not self.breedData.qualityMultiplier[rarity] or  
	...
	local baseStats		= self.breedData.speciesToBaseStats[speciesID]
	
	if not baseStats and not canBattle then return end
...
function lib:GetPetPredictedStats(speciesID, breedID, rarity, level)
	if not self.breedData.breeds[breedID] or not self.breedData.speciesToBaseStats[speciesID] or type(level) ~= "number" or not self.breedData.qualityMultiplier[rarity] then return end
...
function lib:GetSpeciesWithoutProfiles()
	...
	for i=1,2000 do --2000 to catch new pet IDs
		...
		if name and not self.breedData.speciesToBaseStats[i] then
...
--[[function lib:ExtractData()
	...
	local speciesToBaseStats = self.breedData.speciesToBaseStats
	for k,v in pairs(BPBID.BasePetStats) do
		if not speciesToBaseStats[k] then 
			print(k)
		end
	end
end]]
...
lib.breedData.speciesToBaseStats = {
	[1] = false,
	[2] = {510.5, 8, 9.5},
	[3] = false,
	[4] = false,
...
for k,v in pairs(lib.breedData.speciesToBaseStats) do b = b+1 end

missingPets = ""
for k,v in pairs(lib.breedData.speciesToBaseStats) do 
