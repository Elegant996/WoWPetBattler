...
Item
{
                        ...
                        isHitting, isCritting, isProcing)
    {
        ...
        var baseDamageOffset = 5;
        var attackType = PetType.Magic;
        var damageOffset = Math.round(baseDamageOffset + petStage.GetTeam(teamIndex).ActivePet.Power * scaleFactor);

        petStage.GetTeam(teamIndex).ActivePet.DamageOffset += damageOffset;
        petStage.GetTeam(teamIndex).ActivePet.AddAura(435, 5, true, teamIndex, petStage.GetTeam(teamIndex).ActivePetIndex, petStage.GetTeam(teamIndex).ActivePet.Power);
