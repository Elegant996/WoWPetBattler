...
Item
{
                        ...
                        isHitting, isCritting, isProcing)
    {
        ...
        if (!isAvoiding && isHitting)
        {
            ...
            petHelper.CheckCleansingRain(petStage, (teamIndex%2)+1, petStage.GetTeam((teamIndex%2)+1).ActivePetIndex, 500, 4, true, teamIndex, petStage.GetTeam(teamIndex).ActivePetIndex, petStage.GetTeam(teamIndex).ActivePet.Power);
