...
Item
{
                        ...
                        isHitting, isCritting, isProcing)
    {
        ...
        if (!isAvoiding && isHitting)
        {
            ...
            //petStage.GetTeam((teamIndex%2)+1).ActivePet.AddAura(177, 4, true, teamIndex, petStage.GetTeam(teamIndex).ActivePetIndex, petStage.GetTeam(teamIndex).ActivePet.Power);
            petHelper.CheckCleansingRain(petStage, (teamIndex%2)+1, petStage.GetTeam((teamIndex%2)+1).ActivePetIndex, 177, 4, true, teamIndex, petStage.GetTeam(teamIndex).ActivePetIndex, petStage.GetTeam(teamIndex).ActivePet.Power);
