...
Item
{
    ...
    function applyAuraStart(teamIndex, petIndex, auraIndex, duration)
    {
        ...
        var damageOffset = Math.round(baseDamageOffset + petStage.GetTeam(teamIndex).GetPet(petIndex).Power * scaleFactor);

        petStage.GetTeam(teamIndex).GetPet(petIndex).DamageOffset += damageOffset;
    }
