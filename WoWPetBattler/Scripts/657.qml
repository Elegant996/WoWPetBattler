...
Item
{
                        ...
                        isHitting, isCritting, isProcing)
    {
        ...
        if (!isAvoiding && isHitting)
        {
			...
			petHelper.CheckCleansingRain(petStage, teamIndex, petStage.GetTeam(teamIndex).ActivePetIndex, 658, 5, true, petStage.GetTeam(teamIndex).ActivePet.Power);
        }
