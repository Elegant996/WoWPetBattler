...
Item
{
                        ...
                        isHitting, isCritting, isProcing)
    {
        ...
        if (!isAvoiding && isHitting && petStage.GetTeam((teamIndex%2)+1).ActivePet.Type != PetType.Elemental)
        {
            numHits += 1;
            if (isCritting)
                petHelper.CheckDamage(petStage, (teamIndex%2)+1, petStage.GetTeam((teamIndex%2)+1).ActivePetIndex, 1.5*damage, true, false);
            else
        ...
        petStage.GetTeam(0).GetPet(0).AddAura(203, 5, true, teamIndex, petStage.GetTeam(teamIndex).ActivePetIndex, petStage.GetTeam(teamIndex).ActivePet.Power);
